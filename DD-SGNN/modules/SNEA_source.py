...
class SNEA(nn.Module):
    ...
    def forward(self, x, pos_edge_index, neg_edge_index):
        pos_x, neg_x, _, _ = self.first_aggr(x, x, pos_edge_index, neg_edge_index, head=True)
        for i in range(self.layers - 1):
            pos_x, neg_x, _, _ = self.aggrs[i](pos_x, neg_x, pos_edge_index, neg_edge_index, head=True)

        return F.tanh(torch.cat((pos_x, neg_x), dim=1) @ self.W)
