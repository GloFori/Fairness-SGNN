...
class MySGCN(torch.nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int, num_layers: int,  params, lamb: float = 5, bias: bool = True):
        super().__init__()
        ...
        self.conv1 = mySignedConv(in_channels, hidden_channels//2,
                                first_aggr=True, g_sigma= params.g_sigma, c=params.c, ablation=params.ablation)
        self.convs = torch.nn.ModuleList()
        for i in range(num_layers - 1):
            self.convs.append(
                mySignedConv(hidden_channels // 2 , hidden_channels//2,
                           first_aggr=False, g_sigma= params.g_sigma, c=params.c, ablation=params.ablation))
    ...
    def forward(
        ...
        """
        z, m = self.conv1(x, pos_edge_index, neg_edge_index, head)
        z = F.relu(z)
        for conv in self.convs:
            z, m = conv(z, pos_edge_index, neg_edge_index, head)
            z = F.relu(z)
        return z, m
    ...
    def compute_tri_edge_accuracy(
        ...
        for i in range(pos_edge_index.size(1)):
            ...
            if pos_p[i] == 0:
                if degrees[src] >= k and degrees[dst] >= k:
